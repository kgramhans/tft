...
*  for the FFT routine.
*/
std::shared_ptr<FFTParam> InitializeFFT(size_t fftlen)
{
   int temp;
   std::shared_ptr<FFTParam> h{ new FFTParam };
   ...
   */
   h->Points = fftlen / 2;
   h->SinTable.reset(new TF_DATA_TYPE[2*h->Points]);
   h->BitReversed.reset(new int[h->Points]);

   for(size_t i = 0; i < h->Points; i++)
   {
      temp = 0;
      for(size_t mask = h->Points / 2; mask > 0; mask >>= 1)
         temp = (temp >> 1) + (int)(i & mask ? h->Points : 0);
...
std::shared_ptr<FFTParam>  GetFFT(size_t fftlen)
{
