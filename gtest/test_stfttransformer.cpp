...
// Test with a simple forward/backward transform on a time/frequency localized signal
TEST(StftTransformer, ForwardBackward) {
    float overlap = 87.5;
    float nSamples = 256;
    int windowLength = 64;
    float Q = 10.0;
    ...
    ITimeFrequencyTransformer * tft = new StftTransformer(windowLength, overlap);
...
TEST(StftTransformer, Sequenced) {
    float Q = 10.0;
    float overlap = 75.0;
    float nSamples = 1024;
    ...
    ITimeFrequencyTransformer * tft1 = new StftTransformer(windowLength, overlap);
    int nb = tft1->forwardTransform(&vKernel[0], vKernel.size(), 0, 0);
    EXPECT_GT(nb, 0);

    StftTransformer * tft2 = new StftTransformer(windowLength, overlap);
    ...
    for (int i = sequences; i--;) {
        ...
        threadPool.push(thread(&StftTransformer::executeForwardSequence, tft2, i)); //tft2->executeForwardSequence(i);
    }
    ...
    for (int i = sequences; i--;) {
        ...
        threadPool.push(thread(&StftTransformer::executeBackwardSequence, tft2, i));
    }
...
TEST(StftTransformer, TimingSingleThreaded) {
    int octaves = 10;
    double fmax = 0.4;
    float Q = 10.0;
    float overlap = 75.0;
...
TEST(StftTransformer, TimingMultiThreaded) {
    int octaves = 10;
    double fmax = 0.4;
    float Q = 10.0;
    float overlap = 75.0;
    ...
    StftTransformer * tft = new StftTransformer(windowLength, overlap);
    ...
    for (int i = sequences; i--;) {
        ...
        threadPool.push(thread(&StftTransformer::executeForwardSequence, tft, i)); //tft2->executeForwardSequence(i);
    }
    // Join all remaining threads
    while (!threadPool.empty()) {
        threadPool.front().join();
    ...
    for (int i = sequences; i--;) {
        ...
        threadPool.push(thread(&StftTransformer::executeBackwardSequence, tft, i));
    }
...
TEST(StftTransformer, Region) {
    ...
    int halfSize = vw.size() >> 1;
    for (int i = 0; i < vw.size(); i++) {
        vKernel[nSamples / 2 - halfSize + i + 1] = vw[i];
    }
    ...
    region.clear();
    region.push_back(make_pair(-1, -0.1));
    region.push_back(make_pair(-1, 0.5));
    region.push_back(make_pair(nSamples, 0.5));
    region.push_back(make_pair(nSamples, -0.1));
    tft->setPolygonRegion(region);
    nb = tft->backwardTransform(vSignal);
    EXPECT_EQ(nb, vKernel.size());
    for (int i = 0; i < vKernel.size(); i++) {
    ...
    region.clear();
    region.push_back(make_pair(-1, -0.1));
    region.push_back(make_pair(-1, 0.6));
    region.push_back(make_pair(nSamples / 2 - 1, 0.6));
    region.push_back(make_pair(nSamples / 2 + 1, -0.1));
    tft->setPolygonRegion(region);
    nb = tft->backwardTransform(vSignal);
    EXPECT_EQ(nb, vKernel.size());
    double powerRef = 0;
    ...
    region.clear();
    region.push_back(make_pair(2 * nSamples, -0.1));
    region.push_back(make_pair(2 * nSamples, 0.6));
    region.push_back(make_pair(nSamples / 2 - 1, 0.6));
    region.push_back(make_pair(nSamples / 2 + 1, -0.1));
    tft->setPolygonRegion(region);
    nb = tft->backwardTransform(vSignal);
...
TEST(StftTransformer, Normalisation) {
    ...
    while (repeat--) {
        ...
        float overlap = 90.0 - 0.2 * dist100(rng); // Vary in range 70..90
        float nSamples = 1024;
        ...
        EXPECT_LT(SNR, -60)  << std::fixed << std::setw(10) << nb << "\t" << frequency << "\t" << overlap << "\t" << fDelay << "\t" << ratio << "\t" << (int)SNR << "\t" << sqrt(sumOriginal) << "\t" << sqrt(sumRestored)<< "\t";
        delete tft;
    }
}
